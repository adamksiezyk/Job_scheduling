...
class TestScheduler(TestCase):
    def setUp(self) -> None:
        ...
        self.resources_init = Resources(resources)
    ...
    def test_schedule_job_one_resource_is_enough(self):
        ...
        r_idx = self.resources_init.get_resources(j.machine_id).index(self.r2_m1)
        used_resources = {
            'M1': [*self.resources_init.get_resources('M1')],
            'M2': [*self.resources_init.get_resources('M2')]
        }
        used_resources[j.machine_id][r_idx] = r
        self.queue_init.append(s)

        self.scheduler.schedule_job(j)
        self.assertEqual(used_resources['M1'], self.scheduler.resources.get_resources('M1'))
        self.assertEqual(used_resources['M2'], self.scheduler.resources.get_resources('M2'))
    ...
    def test_schedule_job_one_resource_is_not_enough(self):
        ...
        s2 = ScheduledJob(duration=timedelta(hours=4), delay=j.delay, machine_id=j.machine_id, project=j.project,
                          ...
                          previous_machines=j.previous_machines)
        r1 = None
        r2 = replace(self.r3_m2, start_dt=s2.end_dt)
        r1_idx = self.resources_init.get_resources(j.machine_id).index(self.r2_m2)
        r2_idx = self.resources_init.get_resources(j.machine_id).index(self.r3_m2)
        used_resources = {
            'M1': [*self.resources_init.get_resources("M1")],
            'M2': [*self.resources_init.get_resources("M2")]
        }
        used_resources[j.machine_id][r1_idx] = r1
        used_resources[j.machine_id][r2_idx] = r2
